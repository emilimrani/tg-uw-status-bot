...
async def fetch_status(case_no: str, password: str) -> str | tuple[str, bytes]:
    """Returns status text or ('screenshot', image_bytes) if couldn't parse text."""
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Try multiple selectors for the status text
            selectors = [
            ...
            for sel in selectors:
                with suppress(Exception):
                    txt = await page.locator(sel).inner_text(timeout=2500)
            ...
            if body:
                m = re.search(r"(Etap postępowania|Etap postepowania|Status sprawy)\s*[:\-–]\s*(.+)", body, re.I)
                if m:
                    val = m.group(2).split("\n")[0].strip()
                    val = re.sub(r"\s+", " ", val)
                    if val:
